...
#include "Helper.h"
#include "Stage1_Wrapper.h"

#define MAX_BULLETS 10000000

using namespace std;

Stage_Stage1::Stage_Stage1(void)
{
...
void Stage_Stage1::AddNewPlayer()
{
...
void Stage_Stage1::Update(float deltaTime)
{
	SoundManager::GetSingleton().PlayLoop("BGM1");
	NpcManager& npcManager = NpcManager::GetSingleton();
...
#pragma region Solid State Squad
	if ( gameState.State["Clear"] == 1 )
	{
		...
		if ( stageClearTimer > 5 )
			StageManager::GetSingleton().NextStage = "Stage2";
	}
	...
	if ( gameState.State["1up"] == 1 )
	{
		...
		newEnemy->SetSpeed(-100, 0, 0, 0);
		newEnemy->SetPos(800, Helper::Random(0, 45) * 10.0f);
		newEnemy->Health = 200;
		newEnemy->Params["Name"] = "EnemyPlane";
...
void Stage_Stage1::UpdateEntities(Entity *entity, float deltaTime)
{
...
void Stage_Stage1::CollisionCheck(Entity *entity)
{
...
void Stage_Stage1::OnCollide(Entity *a, Entity *b)
{
